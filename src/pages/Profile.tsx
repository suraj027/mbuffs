...
import { FOR_YOU_FULL_PAGE_ITEMS_PER_PAGE, getForYouInfiniteQueryOptions, getForYouRecommendationsQueryKey, getPreferencesQueryKey } from '@/lib/recommendationQueries';
    ...
    const handleToggleRecommendations = (enabled: boolean) => {
        ...
        updatePreferencesMutation.mutate(updateData, {
            onSuccess: () => {
                ...
                if (enabled) {
                    queryClient.invalidateQueries({ queryKey: getForYouRecommendationsQueryKey(user.id) });
                    void queryClient.prefetchInfiniteQuery(getForYouInfiniteQueryOptions(user.id, FOR_YOU_FULL_PAGE_ITEMS_PER_PAGE));
                    return;
                }
